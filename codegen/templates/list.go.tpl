func List{{.Name}}PageSortBy(site string, offset string, sortBy *SortBy) ([]{{.NameSingular}}, string, error) {
	client := fiber.AcquireClient()
	defer fiber.ReleaseClient(client)

	parsedUrl, err := url.Parse("https://" + site + "." + "{{.Url}}")
	if err != nil {
		return nil, "", err
	}
...
func List{{.Name}}Page(site string, offset string) ([]{{.NameSingular}}, string, error) {
	return List{{.Name}}PageSortBy(site, offset, nil)
}
